...
\begin{titlepage}
    \begin{center}
        ...
        \vspace{0.5cm}
        \begin{abstract}
            \noindent A classification analysis and comparison of Decision Trees, Random Forests, Gradient Boosting and Adaptive Boosting as classification methods for classifying heart disease presence is performed. The Cleveland heart disease dataset is used for the classification problem. Prior to applying the classification methods, a Principal Component Analysis (PCA) and the correlation matrix was used to study and better understand which features are most important for the classification and if the features are correlated respectively. The classification results show random forests and boosting methods yielding the best results, while decision trees performed only slightly better than random guessing. AdaBoost, a variant of gradient boosting, achieved the best results overall for the metrics used but gradient boosting and random forests marginally worse. The performance of the classification methods was likely limited by the size of the dataset. A larger dataset could improve the accuracy of the classification methods.
        \end{abstract}
        \vspace{0.8cm}
        \includegraphics[scale=0.2]{heart.jpg}
    \end{center}
\end{titlepage}
